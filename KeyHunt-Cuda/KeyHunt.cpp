...
static uint64_t maxIters = 20000000000000ULL; // 1 триллиона итераций
// ===================================================
	...
	const std::string& rangeStart, const std::string& rangeEnd, bool& should_exit)
{
	...
	this->rKey = 0;
	customStep.SetBase16("384b84d092f01bcce2af43"); // ВАШ ШАГ БЕЗ 0x В НАЧАЛЕ И ещё, строка 160!!!
	// =========================================================================================
	...
	const std::string& rangeStart, const std::string& rangeEnd, bool& should_exit)
{
	...
	this->rangeDiff2.Set(&this->rangeEnd);
	this->rangeDiff2.Sub(&this->rangeStart);
	this->targetCounter = 4294967295; // Настройка количества совпадений 4500; 1200000
	
	    // === МОИ ПРАВКИ: Отключаем рандом и ставим наш шаг ===
    this->rKey = 0; 
    customStep.SetBase16("384b84d092f01bcce2af43"); // ВАШ ШАГ БЕЗ 0x В НАЧАЛЕ!
    // =======================================================
...
{
	// === МОИ ПРАВКИ: Отключаем CPU, чтобы не портил статистику ===
    ph->hasStarted = true;
    endOfSearch = false; 
    ph->isRunning = false;
    return;
...
#define FILTER_SIZE 8
	...
	while (isAlive(params)) {
		...
		t0 = t1;
		// === МОИ ПРАВКИ: Убрали completedPerc > 100.5, иначе программа выключится ===
