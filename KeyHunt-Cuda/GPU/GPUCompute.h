...
// ------------------Изменяем префикс 0x61DB9B9F------------------------------------------

__device__ __noinline__ bool MatchXPoint(uint32_t* _h, uint32_t* xpoint)
{
    // Префикс 61DB9B9F (8 hex-символ = 4 байта)
    const uint8_t target[4] = {0x61, 0xDB, 0x9B, 0x9F}; // Тоже менять [4] 
    const uint8_t* h8 = (const uint8_t*)_h;

    for (int i = 0; i < 4; i++) { // Тоже менять 4;
        if (h8[i] != target[i]) {
            return false;
        }
    }
